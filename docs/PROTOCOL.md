...
message WriteRequest {
  ...
  string hostname = 4;      // Client hostname for debugging and origin tracking
  int64 timestamp = 5;      // Unix timestamp (seconds since epoch) when write was initiated
}
```
...
message DataFrame {
  ...
  string hostname = 4;      // Hostname of client that wrote this data (for debugging)
  int64 timestamp = 5;      // Timestamp when data was written (for debugging)
}
```
...
### Debug Information
To aid in debugging and monitoring clipboard operations, the protocol includes optional metadata:

**Hostname tracking:**
- Client automatically includes its hostname in every `WriteRequest`
- Server stores and forwards the hostname with clipboard data
- Visible in `DataFrame` messages when using debug mode (`-vv`)
- Helps identify which machine originated clipboard content in multi-host setups

**Timestamp tracking:**
- Client includes Unix timestamp (seconds since epoch) in every `WriteRequest`
- Server preserves and forwards the timestamp with clipboard data
- Available in `DataFrame` messages when using debug mode
- Useful for tracking when clipboard content was created

**Debug mode usage:**
```bash
# Enable debug logging to see hostname and timestamp
tty-cb-client -vv read localhost
# Output includes:
# [DEBUG] Data from host: workstation1, timestamp: 1733289600

# Bridge scripts support debug mode with -d flag
./wayland-bridge.sh -d localhost 5457
# Shows origin information for all clipboard updates
```

The debug information does not affect clipboard functionality and can be safely ignored by clients that don't need it.
